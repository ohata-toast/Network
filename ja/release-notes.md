...
#### 機能改善
##### Load Balancer
* [韓国/日本/米国リージョン]ロードバランサーと同じVPCに属している他のサブネットのインスタンスを、ロードバランサーのメンバーに登録できます。ロードバランサーが属しているサブネットとインスタンスのサブネットがRouting Tableに接続されている必要があります。
* [韓国/日本/米国リージョン]ロードバランサーが属しているVPCがPeering接続されている場合、Peer VPCに属しているインスタンスをロードバランサーのメンバーに登録できます。  Peer VPCのDefault Routing Tableに接続されたサブネット上のインスタンスのみ接続できます。
